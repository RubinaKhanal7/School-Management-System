    ...
    public function getStudentsDetails(Request $request)
{
    ...
    public function getStudentMarksForEdit(Request $request)
    {
        try {
            $examScheduleId = $request->input('examination_schedule_id');
            $subjectId = $request->input('subject_id');
   
            $studentsMarks = ExamResult::where('exam_schedule_id', $examScheduleId)
                ->where('subject_id', $subjectId)
                ->with([
                    'studentSession.student:id,admission_no,roll_no,user_id',
                    'studentSession.student.user:id,f_name,m_name,l_name'
                ])
                ->get();
            ...
            if ($studentsMarks->isEmpty()) {
                return response()->json([
                    'status' => 'error',
                    'message' => 'No marks found for this configuration'
                ], 404);
            }
           
            $html = view('backend.school_admin.examination.results.update', [
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'Error fetching marks: ' . $e->getMessage()
            ], 500);
        }
    }
        ...
        public function update(Request $request)
        {
            try {
                ...
                foreach ($request->input('student_session_id') as $key => $studentSessionId) {
                    // Find or create the exam result
                    ...
                    if (!$examResult) {
                        ...
                        $examResult->exam_schedule_id = $request->exam_schedule_id;
                        $examResult->student_session_id = $studentSessionId;
                        $examResult->subject_id = $request->subject_id;
                    }
       
                    // Update marks and other fields
                    $examResult->attendance = isset($request->attendance_check[$key]) ? 0 : 1; // Checkbox unchecked means present (1)
                    $examResult->participant_assessment = $request->participant_assessment[$key] ?? $examResult->participant_assessment;
                    $examResult->practical_assessment = $request->practical_assessment[$key] ?? $examResult->practical_assessment;
                    $examResult->theory_assessment = $request->theory_assessment[$key] ?? $examResult->theory_assessment;
                    $examResult->notes = $request->notes[$key] ?? $examResult->notes;
                    $examResult->is_active = 1;
